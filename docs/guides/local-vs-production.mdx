...
Open your browser → navigate to your app → check the **Console** (F12):

```
[analytiq] Initialized successfully.
```

Then go to your Analytiq Dashboard → Events → your events appear within **5-10 seconds**.
...
| Redeployed but env var not picked up | Go to Vercel → Deployments → trigger a new manual deploy |
